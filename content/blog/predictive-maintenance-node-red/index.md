...
flowchart LR
    ...
    C --> D["Health Index<br/>Calculation"]
    D --> E["Trend Analysis"]
    E --> F["RUL Prediction"]
    F --> G["Alert Engine"]
...
flowchart LR
    ...
    BUF --> PFFT[Python FFT]
    PFFT --> FE[Feature Extract]
    OPC["OPC-UA<br/>temperature"] --> MF[Merge Features]
    MOD["Modbus<br/>motor amps"] --> MF
    FE --> MF
    MF --> HI[Health Index]
    HI --> IDB[InfluxDB Write]
    HI --> DG[Dashboard Gauge]
    HI --> AC[Anomaly Check]
    AC --> ZS[Z-Score]
    AC --> CU[CUSUM]
    AC --> IF[Isolation Forest]
    ZS --> AD[Alert Decision]
    CU --> AD
    IF --> AD
    AD --> EM[Email]
...
| Health Index | Status | Action |
|--------------|--------|--------|
| ≥ 80 % | Green | No action |
| 50–79 % | Yellow | Log to CMMS, notify maintenance planner |
| 20–49 % | Orange | Priority work order, order spare parts |
| < 20 % | Red | Immediate attention, schedule emergency stop |
