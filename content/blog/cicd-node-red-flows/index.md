...
flowchart TB
    subgraph NR["Node-RED Instance (Server) — ~/.node-red/"]
        ...
        C["flows_cred.json — encrypted credentials"]
        S["settings.js — runtime config"]
        P["package.json — installed nodes"]
...
flowchart LR
    ...
    B["Build & Push<br/>Docker · ghcr.io"]
    S["Deploy Staging<br/>Auto"]
    P["Deploy Production<br/>Manual approval"]
    L --> T --> B --> S --> P
    S -. Health check .-> S
    P -. Health check .-> P
{{< /mermaid >}}
