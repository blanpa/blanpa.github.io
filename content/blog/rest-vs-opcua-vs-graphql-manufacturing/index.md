...
flowchart LR
    ...
    Obj --> Srv["Server<br/>(metadata, diagnostics)"]
    Obj --> Fac["Factory"]
    Fac --> L1["Line1"]
    Fac --> L2["Line2"]
    L1 --> C1["CNC-001"]
    L1 --> C2["CNC-002"]
    C1 --> T["Temperature<br/>Double, EURange 0–200 °C"]
    C1 --> SS["SpindleSpeed<br/>Double, EURange 0–12000 RPM"]
    C1 --> St["Status<br/>Int32, Enum 0=Off / 1=Run / 2=Err"]
    C1 --> PP["PartsProduced<br/>Int64"]
    C1 --> Rst["Reset() — Method"]

    Tps --> CMT["CNCMachineType"]
    CMT --> CT["Temperature<br/>BaseDataVariableType, Double"]
    CMT --> CS["SpindleSpeed<br/>AnalogItemType, Double"]
...
sequenceDiagram
    ...
    C->>S: GET /temperature
    S-->>C: { value: 42.7 } (wasted, no change)
    Note right of C: wait 1s
    C->>S: GET /temperature
    S-->>C: { value: 43.1 } (change detected ≤1s late)

    Note over C,S: OPC-UA (Subscription)
    C->>S: CreateSubscription
    C->>S: MonitoredItemCreate
    S-->>C: Publish(42.7)  initial value
    Note over S: server monitors internally
    S-->>C: Publish(43.1)  on change
    S-->>C: Publish(43.8)  on change

    Note over C,S: GraphQL (WebSocket Subscription)
    C->>S: WS subscribe { temp }
...
flowchart LR
    ...
    Root --> T["Temperature"]
    Root --> SS["SpindleSpeed"]
    Root --> St["Status"]

    T --> TV["Value: 42.7 (Double)"]
    T --> TU["EngineeringUnits: °C"]
    T --> TR["EURange: 0.0 – 200.0"]
    T --> TI["InstrumentRange: -40.0 – 250.0"]
    T --> TD["Description: 'Coolant temperature'"]

    SS --> SV["Value: 2450.0 (Double)"]
    SS --> SU["EngineeringUnits: RPM"]
    SS --> SR["EURange: 0.0 – 12000.0"]
...
flowchart TB
    subgraph SF["Shop Floor"]
        direction LR
        P1[PLC] -->|OPC-UA| EG1[Edge Gateway]
        P2[PLC] -->|OPC-UA| EG2[Edge Gateway]
        EG1 -->|MQTT| BR[Broker]
        EG2 -->|MQTT| BR
    end
    ...
    subgraph SR["Server Room"]
        direction LR
        DP["Data Platform<br/>(Node-RED / Custom)"]
        H["Historian<br/>(OPC-UA HDA)"]
        REST["REST API<br/>(External)"]
        GQL["GraphQL API<br/>(Aggregation)"]
        DP --> H
        DP --> REST
        DP --> GQL
    end
    ...
    subgraph CL["Cloud"]
        direction LR
        AGW["API Gateway<br/>(REST / GraphQL)"]
