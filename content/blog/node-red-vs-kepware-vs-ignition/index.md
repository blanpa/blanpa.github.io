...
flowchart TB
    subgraph ig["Ignition"]
        direction TB
        V["Vision<br/>(Desktop)"] --> TDB
        P["Perspective<br/>(Web UI)"] --> TDB
        R["Reporting Module"] --> H
...
flowchart LR
    ...
    subgraph ESR["Edge / Server Room"]
        KEP["Kepware KEPServer<br/>(PLC Drivers)"]
        NR["Node-RED<br/>(Transform + Route)"]
        GR["Grafana + PostgreSQL<br/>(IT-side dashboards)"]
    end

    subgraph CE["Cloud / Enterprise"]
        IGN["Ignition SCADA<br/>HMI · Historian · Alarming · Reporting"]
        CA["Cloud Analytics<br/>(Azure / AWS)"]
    end
...
flowchart LR
    ...
    subgraph NR["Node-RED"]
        CU["Cloud Upload<br/>(Azure IoT Hub)"]
        ERP["ERP Integration<br/>(SAP REST API)"]
        CR["Custom Reports<br/>(Email + PDF)"]
    end
