...
| Package                | Version |
| ---------------------- | ------- |
| `condition-monitoring` | v0.3.1  |
| `nats-suite`           | v0.2.2  |
| `opcua-suite`          | v0.1.7  |
| `cip-suite`            | v0.0.5  |
| `s7-suite`             | v0.0.7  |
| `i3x`                  | v0.0.8  |
| `clab-interfaces`      | v0.0.1  |
...
flowchart LR
    ...
    subgraph AFTER["After (v0.3) — 5–8 config fields each, clear purpose"]
        CONN["opcua-connection<br/>(config node)"]
        BR["opcua-browse"]
        RD["opcua-read"]
        WR["opcua-write"]
        SUB["opcua-subscribe"]
...
flowchart LR
    subgraph FLOW["Node-RED Flow — 3 operation nodes, 1 shared connection"]
        ...
        SUB["nats-sub"]
        REQ["nats-req"]
        CONN["nats-conn (config)<br/>host: ...<br/>port: ...<br/>creds: ..."]
        PUB --> CONN
        SUB --> CONN
        REQ --> CONN
    end
...
| Fast network         | Satellite link with 800 ms latency          |
