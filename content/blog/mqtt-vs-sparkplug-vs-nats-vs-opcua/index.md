...
flowchart LR
    Root --> Objects
    Root --> Types
    Objects --> Server["Server<br/>(diagnostics, capabilities)"]
    Objects --> Factory
    Factory --> Line1
    Factory --> Line2
    Line1 --> Machine3
    Line1 --> Machine4
    Machine3 --> Temp["Temperature<br/>Float, °C, 0–200"]
    Machine3 --> Speed["Speed<br/>Int32, RPM, 0–3000"]
    Machine3 --> Status["Status<br/>Enum: Running / Stopped / Error"]
    Machine3 --> Methods
    Methods --> Start["Start()"]
    Methods --> Stop["Stop()"]
...
flowchart TB
    ...
    Q --> PLC["PLC Integration<br/>(automation)"]
    Q --> HT["High-Throughput<br/>Data Pipeline"]

    ST --> SS{"Simple?"}
    ST --> SC{"Need SCADA<br/>standard?"}
    SS -->|yes| MQ1["MQTT"]
    SS -->|no| SP1["Sparkplug B"]
    SC -->|yes| SP2["Sparkplug B"]
    SC -->|no| MQ2["MQTT"]

    PLC --> OPC["OPC-UA"]

    HT --> P{"Need<br/>Persistence?"}
    HT --> CN{"Cloud-<br/>native?"}
    P -->|yes| NJ["NATS<br/>(JetStream)"]
    P -->|no| MQ3["MQTT (+DB)"]
    CN -->|yes| N1["NATS"]
    CN -->|no| N2["NATS"]
{{< /mermaid >}}
