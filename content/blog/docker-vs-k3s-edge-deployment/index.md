...
flowchart TB
    subgraph wo["Without Containers — Industrial Edge PC"]
        direction TB
        wo1["Node-RED v3.1<br/>(needs Node.js 18)"]
        wo2["Grafana v10.2<br/>(needs libfontconfig)"]
        wo3["PostgreSQL 15<br/>(port conflict with legacy)"]
        wo4["NATS Server<br/>(manual systemd service)"]
        wo5["Python 3.11<br/>(but script needs 3.9)"]
        wos["State: 'works on my machine' / 'don't touch it'"]
        style wos fill:none,stroke:none
    end

    subgraph wi["With Containers — Industrial Edge PC"]
        direction TB
        wi1["Node-RED 3.1"]
        wi2["Grafana 10.2"]
        wi3["PostgreSQL 15"]
...
flowchart TB
    subgraph br["Docker Bridge Network — docker0 (172.17.0.0/16)"]
        ...
        NR --- NA
        NA --- GR
        SD["Service discovery<br/>via container names"]
        style SD fill:none,stroke:none
    end

    subgraph host["Host Network"]
        direction LR
        E0["eth0: 192.168.1.50<br/>← factory LAN"]
        E1["eth1: 10.0.0.50<br/>← PLC network (isolated)"]
    end
...
| Multiple sites, fleet management | K3s + Rancher |
| HA requirement (99.9%+) | K3s (multi-server) |
| Raspberry Pi / 2 GB RAM | Docker Compose |
| Industrial PC / 8 GB+ RAM | Either (your choice) |
